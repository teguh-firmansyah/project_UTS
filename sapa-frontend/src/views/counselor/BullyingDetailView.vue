...
const handleUpdateStatus = async () => {
  ...
  try {
    ...
    if (validationErrors) {
      toast.error(Object.values(validationErrors)[0]?.[0] || 'Periksa kembali data yang diisi.')
    } else {
      toast.error(error?.response?.data?.message || 'Gagal memperbarui laporan. Silakan coba lagi.')
    }
  } finally {
...
async function handleRevealIdentity() {
  ...
  try {
    ...
    toast.error(error?.response?.data?.message || 'Gagal membuka identitas pelapor.')
  } finally {
...
const priority = computed(() => report.value ? getPriority(report.value.priority) : getPriority('Rendah'))
    ...
    <template v-else-if="report">
        ...
        <div class="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <div class="min-w-0 space-y-6">
            ...
            <section class="print-card fade-up relative overflow-hidden rounded-xl border border-slate-800 bg-slate-900 shadow-xl shadow-black/25">
              ...
              <div class="relative z-10 space-y-6 p-5 sm:p-6 lg:p-7">
                ...
                <div>
                  <div v-if="report.is_anonymous" class="flex items-start gap-3 rounded-xl border border-rose-500/25 bg-rose-500/[0.04] p-4">
                    ...
                    <div class="min-w-0">
                      ...
                      <p class="mt-1.5 text-xs leading-relaxed text-slate-400">
                        Siswa mengajukan laporan ini secara rahasia. Peran pelapor: <span class="font-semibold text-slate-200">{{ report.reporter_relation }}</span>.
                      </p>
                      <p class="mt-2 text-[11px] leading-relaxed text-rose-300/80">
                        Identitas pelapor anonim tidak dapat dibuka melalui sistem — sesuai kebijakan perlindungan data pelapor.
                      </p>
                    </div>
                  </div>
                  ...
                  <div v-else class="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
                    <div class="flex items-center justify-between gap-3 border-b border-slate-800/70 pb-3">
                      ...
                      <span v-if="report.reporter_name" class="inline-flex items-center gap-1.5 text-[11px] font-semibold text-emerald-400">
                    ...
                    <div v-if="report.reporter_name" class="mt-4 flex items-center gap-3.5">
                    ...
                    <div v-else class="mt-4 flex items-center justify-between gap-3 rounded-lg border border-amber-500/20 bg-amber-500/[0.04] px-3.5 py-3">
                      <p class="text-xs text-slate-400">Identitas pelapor belum ditampilkan.</p>
                      <button
                        type="button"
                ...
                <div v-if="report.attachments.length > 0" class="space-y-3 border-t border-slate-800/70 pt-5">
                  <div class="flex items-center gap-2">
                    ...
                    <p class="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">Bukti Pendukung ({{ report.attachments.length }} berkas)</p>
                  </div>
                  ...
                  <div class="flex flex-wrap gap-3">
                    <figure
                      ...
                      @click="att.file_type?.startsWith('image/') && openImage(att.url)"
                    >
                      <img
                      ...
                      />
                      <div v-else class="flex h-32 w-full items-center justify-center bg-slate-900 text-[11px] font-semibold text-slate-500">
                        Berkas PDF
                      </div>
                      <span v-if="att.file_type?.startsWith('image/')" class="pointer-events-none absolute inset-0 flex items-center justify-center gap-2 bg-slate-950/50 text-[10px] font-semibold text-white opacity-0 backdrop-blur-[2px] transition-opacity duration-200 group-hover:opacity-100 print:hidden">
                        Perbesar
                      </span>
                    </figure>
                  </div>
            ...
            <section class="print-card fade-up overflow-hidden rounded-xl border border-slate-800 bg-slate-900" style="animation-delay: 90ms">
              ...
              <p v-else class="px-5 py-8 text-center text-xs text-slate-500 sm:px-6">Belum ada riwayat penanganan.</p>
          ...
          <aside class="space-y-6 print:hidden lg:sticky lg:top-20">
            <section class="fade-up overflow-hidden rounded-xl border border-slate-800 bg-slate-900 shadow-xl shadow-black/25" style="animation-delay: 140ms">
              ...
              <div v-if="['Selesai', 'Ditolak'].includes(report.status)" class="p-5">
                <div class="flex items-start gap-3 rounded-lg border border-slate-700 bg-slate-950/50 px-3.5 py-3">
                  <svg class="mt-0.5 h-4 w-4 shrink-0 text-slate-500" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <p class="text-xs leading-relaxed text-slate-400">
                    Kasus ini sudah berstatus <span class="font-semibold text-slate-200">{{ report.status }}</span> dan tidak dapat diubah lebih lanjut dari halaman ini.
                  </p>
                </div>
              </div>

              <form v-else @submit.prevent="handleUpdateStatus" class="space-y-5 p-5">
      ...
      >
        ...
        <div class="modal-panel relative w-full max-w-4xl overflow-hidden rounded-xl border border-slate-800 bg-slate-900 p-2 shadow-2xl shadow-black/50">
          ...
          <img :src="activeAttachmentUrl" alt="Bukti laporan diperbesar" class="max-h-[80vh] w-full rounded-lg object-contain" />
