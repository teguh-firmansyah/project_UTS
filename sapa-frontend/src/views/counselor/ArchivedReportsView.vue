...
async function loadArchive() {
  ...
  try {
    const data = await reportService.getBullyingArchive()

    archivedReports.value = data.data.map((r) => ({
      id: r.id,
      ticket_code: r.report_code,
      category: r.title,
      reporter_name: r.is_anonymous ? 'Anonim' : (r.reporter?.name ?? '—'),
      is_anonymous: r.is_anonymous,
      reporter_class: r.is_anonymous ? '—' : (r.reporter?.class_name ?? '—'),
      created_at: new Date(r.created_at).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }),
      closed_at: r.resolved_at
        ? new Date(r.resolved_at).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })
        : '—',
    ...
    toast.error('Gagal memuat arsip laporan.')
  } finally {
...
const goToQueue = () => router.push({ name: 'counselor-bullying-queue' })
    ...
    <main class="mx-auto w-full max-w-7xl flex-1 space-y-8 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      ...
      <div v-if="isLoading" class="space-y-8">
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
          <div v-for="i in 3" :key="i" class="h-28 rounded-xl border border-slate-800 bg-slate-900 animate-pulse" />
        </div>
        <div class="rounded-xl border border-slate-800 bg-slate-900 p-6">
          <div v-for="i in 3" :key="i" class="h-16 border-b border-slate-800/50 animate-pulse" />
        </div>
      </div>
    ...
    >
      ...
      <div class="modal-panel relative flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 shadow-2xl shadow-black/50">
        ...
        <div class="modal-scroll min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
          ...
          <div class="space-y-2">
            <p class="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">Catatan Penanganan</p>
...
@media (prefers-reduced-motion: reduce) {
  .fade-up, .card-enter, .stat-bar, .backdrop-in, .modal-panel { animation: none; opacity: 1; }
}
</style>