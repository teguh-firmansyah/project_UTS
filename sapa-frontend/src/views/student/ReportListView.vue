...
const fetchReports = async () => {
  ...
  try {
    ...
    isError.value = true
    errorMessage.value = err.response?.data?.message || 'Gagal memuat daftar laporan dari server.'
  } finally {
  ...
  <div class="sapa-root flex min-h-screen flex-col bg-slate-950 font-sans text-slate-100 antialiased selection:bg-emerald-500/25">
    ...
    <main class="mx-auto w-full max-w-7xl flex-1 space-y-8 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      ...
      <section class="fade-up overflow-hidden rounded-xl border border-slate-800 bg-slate-900" style="animation-delay: 90ms">
        ...
        <div v-if="isLoading" class="flex flex-col items-center justify-center py-16 text-slate-500">
          <Loader2 class="h-8 w-8 animate-spin text-emerald-500" />
          <p class="mt-3 text-xs font-medium">Memuat riwayat laporan...</p>
        </div>

        <div v-else-if="isError" class="flex flex-col items-center px-6 py-12 text-center">
          <div class="flex h-12 w-12 items-center justify-center rounded-xl border border-red-500/20 bg-red-500/10 text-red-400">
            <AlertTriangle class="h-6 w-6" />
          </div>
          <p class="mt-3 text-sm font-semibold text-slate-200">{{ errorMessage }}</p>
          <button
            type="button"
            @click="fetchReports"
            class="mt-4 inline-flex items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-800 px-4 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-700 active:scale-[.97]"
          >
            Coba Lagi
          </button>
        </div>
        ...
        <div v-else-if="reportRows.length > 0" class="divide-y divide-slate-800/70">
          ...
          >
            <span class="absolute bottom-3 left-0 top-3 w-0.75 rounded-r-full opacity-0 transition-opacity duration-200 group-hover:opacity-100" :class="row.accent" aria-hidden="true"></span>

            <div class="min-w-0">
              <span class="rounded border border-emerald-500/20 bg-emerald-500/10 px-1.5 py-0.5 font-mono text-[11px] font-medium text-emerald-400">{{ row.report_code || 'REP-' + row.id }}</span>
