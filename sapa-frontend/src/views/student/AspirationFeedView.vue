...
import {
  ArrowLeft, Lightbulb, TrendingUp, CheckCircle2, Search, X,
  ChevronsUpDown, ChevronDown, Lock, ArrowUp, Plus,
...
const statItems = computed(() => [
  { label: 'Total Aspirasi', value: totalAspirations.value, icon: Lightbulb, tile: 'border-purple-500/25 bg-purple-500/10 text-purple-400', num: 'text-white' },
  { label: 'Total Dukungan', value: totalVotes.value, icon: TrendingUp, tile: 'border-emerald-500/25 bg-emerald-500/10 text-emerald-400', num: 'text-emerald-400' },
  { label: 'Anda Dukung', value: likedCount.value, icon: CheckCircle2, tile: 'border-purple-500/25 bg-purple-500/10 text-purple-400', num: 'text-purple-400' },
])
  ...
  <div class="sapa-root flex min-h-screen flex-col bg-slate-950 font-sans text-slate-100 antialiased selection:bg-emerald-500/25">
    ...
    <main class="mx-auto w-full max-w-7xl flex-1 space-y-8 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      ...
      <section class="fade-up relative overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
        ...
        <span class="absolute inset-x-0 top-0 z-10 h-0.5 bg-linear-to-r from-purple-500/70 via-purple-500/20 to-transparent" aria-hidden="true"></span>
      ...
      <section class="fade-up space-y-4" style="animation-delay: 90ms">
        ...
        <div class="flex flex-col gap-3 sm:flex-row">
          ...
          <div class="relative w-full sm:w-56">
            <ChevronsUpDown class="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" />
            <select
              v-model="sortMode"
      ...
      <div v-if="isLoading" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
        <div v-for="i in 6" :key="i" class="h-52 rounded-xl border border-slate-800 bg-slate-900 animate-pulse" />
      </div>
      ...
      <template v-else>
        ...
        >
          <span class="absolute bottom-4 left-0 top-4 w-0.75 rounded-r-full bg-purple-500/70 opacity-0 transition-opacity duration-200 group-hover:opacity-100" aria-hidden="true"></span>
