...
<template>
  <div class="min-h-screen bg-slate-900 flex items-center justify-center p-4 sm:p-6 overflow-hidden">
    ...
    <div class="animate-fade-up w-full max-w-4xl bg-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row border border-slate-700/80 transition-all duration-500">
      ...
      <div class="relative md:w-1/2 bg-gradient-to-br from-emerald-900 via-slate-900 to-slate-950 p-8 flex flex-col justify-between overflow-hidden min-h-[300px] md:min-h-[600px]">
        ...
        <div class="relative z-10 my-auto py-8">
          <h2 class="text-2xl md:text-3xl font-bold text-white leading-tight mb-2">
            Buat Akun Siswa,<br />Sampaikan Aspirasimu.
          </h2>
      ...
      <div class="md:w-1/2 p-8 lg:p-10 flex flex-col justify-between bg-slate-800">
        <div>
          ...
          <div class="text-center mb-6">
            ...
            <p class="text-xs text-slate-400 mt-1">Lengkapi data diri siswa untuk mendaftar</p>
          </div>

          <form @submit="onSubmit" class="space-y-3.5">
            <!-- Input Nama Lengkap -->
            <div>
              ...
              <input
                ...
                class="w-full bg-slate-700/40 border border-slate-600/80 rounded-lg px-3.5 py-2 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all duration-300"
              />
              <p v-if="errors.name" class="text-xs text-red-400 mt-1 animate-fade-in">{{ errors.name }}</p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-xs font-medium text-slate-300 mb-1">NIS</label>
                <input
                  ...
                  class="w-full bg-slate-700/40 border border-slate-600/80 rounded-lg px-3.5 py-2 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all duration-300"
                />
                <p v-if="errors.identity_number" class="text-xs text-red-400 mt-1 animate-fade-in">{{ errors.identity_number }}</p>
              </div>
              <div>
                <label class="block text-xs font-medium text-slate-300 mb-1">Kelas</label>
                <input
                  v-model="class_name"
                  v-bind="classAttrs"
                  type="text"
                  ...
                  class="w-full bg-slate-700/40 border border-slate-600/80 rounded-lg px-3.5 py-2 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all duration-300"
                />
                <p v-if="errors.class_name" class="text-xs text-red-400 mt-1 animate-fade-in">{{ errors.class_name }}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="relative">
                <input
                  ...
                  placeholder="••••••••"
                  class="w-full bg-slate-700/40 border border-slate-600/80 rounded-lg pl-3.5 pr-10 py-2 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all duration-300"
                />
                <button
                  type="button"
            ...
            <div>
              <label class="block text-xs font-medium text-slate-300 mb-1">Konfirmasi Password</label>
              <div class="relative">
                <input
              ...
              <p v-if="errors.password_confirmation" class="text-xs text-red-400 mt-1 animate-fade-in">{{ errors.password_confirmation }}</p>
            </div>
            ...
            <button
              ...
              class="w-full bg-emerald-500 hover:bg-emerald-600 active:scale-[0.98] text-slate-950 font-semibold rounded-lg py-2.5 text-sm transition-all duration-200 shadow-md shadow-emerald-500/10 disabled:opacity-50 mt-3"
            >
        ...
        <!-- Link Login di Paling Bawah Form -->
        <div class="mt-6 text-center pt-4 border-t border-slate-700/50">
          <p class="text-xs text-slate-400">
            Sudah punya akun?
            <router-link to="/login" class="text-emerald-400 hover:text-emerald-300 hover:underline font-medium transition-colors ml-1">
              Masuk sekarang
            </router-link>
          </p>
        </div>
